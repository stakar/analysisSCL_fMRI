...
### I comment this function, lots of blunders (medfilter must be changed, data is always converting same file etc.
...
def calculate_save_results_bsl(signal,
    ...
    for part in parts:
        for condition in conditions:
            ...
            triggers = list(map(lambda x: bool(re.match(regexp, x)), events))
            triggers = events_times[triggers][0::2]
            bsl_name = events[list(map(lambda x: bool(re.match("S [1-9][1-9]$", x)), events))][0]
